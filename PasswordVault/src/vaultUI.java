 ...
 * 			Password-Vault
 * 
 * 			-This class will create a GUI application layer for the Password-Vault.
...
public class vaultUI extends Application{
	
	password_vaultGUI pv = new password_vaultGUI();
	Stage window;
				...
				public void handle(ActionEvent event) {
					...
					if(pv.returnVaultUI().isEmpty()) {
						try {
							...
							Image image = new Image(new FileInputStream("empty.png"));
...
// method for Logo's VBox
		public VBox logoVBox() {
...
/// method for displaying logo on each scene
		public VBox makeLogo() {
			Image logo;
			ImageView holdLogo;
			VBox frameLogo = logoVBox();
			frameLogo.setAlignment(Pos.TOP_CENTER);
			
			try {
				logo = new Image(new FileInputStream("welcome.png"));
...
// set main to window.		
		window.setScene(main);
		window.setTitle("Project: Password-Vault");
// centering window		
